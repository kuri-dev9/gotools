...
// deadlineConn turns the dial timeout into an idle timeout for established
// data connections as well. A successful dial must not leave LIST/RETR/STOR
// reads or writes able to block indefinitely.
...
func withDeadline(conn net.Conn, timeout time.Duration) net.Conn {
	if timeout <= 0 {
		return conn
	}
...
func (c *ServerConn) openDataConn() (net.Conn, error) {
	...
	if c.options.dialFunc != nil {
		...
		return withDeadline(conn, c.options.dialer.Timeout), nil
	}
	...
	if c.options.tlsConfig != nil {
		...
		return withDeadline(tls.Client(conn, c.options.tlsConfig), c.options.dialer.Timeout), err
	}
...
func (c *ServerConn) openActiveDataListener() (*net.TCPListener, error) {
	localAddr, ok := c.netConn.LocalAddr().(*net.TCPAddr)
	if !ok {
		return nil, fmt.Errorf("FTP control connection has unsupported local address %q", c.netConn.LocalAddr())
	}
	listener, err := net.ListenTCP("tcp", &net.TCPAddr{IP: localAddr.IP, Port: 0})
	if err != nil {
		return nil, err
	}
	listenAddr := listener.Addr().(*net.TCPAddr)
	if ip := listenAddr.IP.To4(); ip != nil {
		_, _, err = c.cmd(StatusCommandOK, "PORT %d,%d,%d,%d,%d,%d",
			ip[0], ip[1], ip[2], ip[3], listenAddr.Port/256, listenAddr.Port%256)
	} else {
		_, _, err = c.cmd(StatusCommandOK, "EPRT |2|%s|%d|", listenAddr.IP.String(), listenAddr.Port)
	}
	if err != nil {
		_ = listener.Close()
		return nil, err
	}
	if c.options.dialer.Timeout > 0 {
		if err := listener.SetDeadline(time.Now().Add(c.options.dialer.Timeout)); err != nil {
			_ = listener.Close()
			return nil, err
		}
	}
...
func (c *ServerConn) acceptActiveDataConn(listener *net.TCPListener) (net.Conn, error) {
...
func (c *ServerConn) cmdDataConnFrom(offset uint64, format string, args ...interface{}) (net.Conn, error) {
	...
	if c.options.activeMode {
		listener, err = c.openActiveDataListener()
	} else {
	...
	if listener != nil {
		conn, err = c.acceptActiveDataConn(listener)
		if err != nil {
			return nil, err
		}
	}
